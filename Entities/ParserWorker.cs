...
public sealed class ParserWorker : IDisposable
{
    private BrowserClient Browser { get; }
    ...
    public async Task<JToken?> GetJsonFromLinkAsync(string link)
    {
        try
        {
            var scripts = await Browser.GetScriptsFormPageAsync(link).ConfigureAwait(false);
    ...
    {
        Browser = new BrowserClient(proxy);
